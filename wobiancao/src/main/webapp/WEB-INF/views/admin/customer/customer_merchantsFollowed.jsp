	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">用户关注的商户</h4>
			</div>
			<div class="modal-body">
				<div>
					<c:choose>
						<c:when test="${customer.merchantsFollowed.size() == 0}">
						...
						<c:otherwise>
							<table class="table table-bordered">
								...
								<c:forEach items="${customer.merchantsFollowed}" var="item" varStatus="itemStatus">
									<tr>
										<td>${itemStatus.count}</td>
										<td>${item.id}</td>
										<td>${item.name}</td>
										<td><img src="${item.logo}" height="30px"></td>
									</tr>
								</c:forEach>
							</table>
						</c:otherwise>
